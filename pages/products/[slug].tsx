...
import { SEO_PRODUCTS } from "../../src/data/seoContent";
import type { Product } from "../../src/data/site";

interface Props { onAdd:(p:Product)=>void; justAddedId:string|null; }
export default function ProductDetail({onAdd,justAddedId}:Props){
  const router=useRouter();
  const product=SEO_PRODUCTS.find(p=>p.slug===router.query.slug);
  if(!product) return <section className="empty-page shell"><h1>محصول پیدا نشد</h1><Link href="/products">بازگشت به محصولات</Link></section>;
  const canonical=`https://samadifarm.ir/products/${product.slug}`;
  return <>
    <Head><title>{product.seoTitle}</title><meta name="description" content={product.seoDescription}/><link rel="canonical" href={canonical}/><meta property="og:title" content={product.seoTitle}/><meta property="og:description" content={product.seoDescription}/><meta property="og:image" content={product.img}/><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify({"@context":"https://schema.org","@type":"Product",name:product.name,image:[product.img],description:product.seoDescription,offers:{"@type":"Offer",price:product.price,priceCurrency:"IRR",availability:"https://schema.org/InStock",url:canonical}})}}/></Head>
