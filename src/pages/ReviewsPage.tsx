...
import { submitReview, type ApiReview, type ReviewRatingSummary } from "../lib/reviewsApi";

interface ReviewsPageProps { reviews: ApiReview[]; ratingSummary: ReviewRatingSummary | null; }
...
export default function ReviewsPage({ reviews, ratingSummary }: ReviewsPageProps) {
  ...
  // The full review list above is sourced from the API; do not mix in demo testimonials.
  const TESTIMONIALS: Testimonial[] = [];
  const RATING_SUMMARY = { avg: toFa((ratingSummary?.average ?? 0).toFixed(1)), total: Math.max((ratingSummary?.count ?? 0) - reviews.length, 0) };
  const RATING_DIST = ratingSummary?.distribution?.map(({stars, percentage}) => ({stars, pct: percentage})) ?? [5,4,3,2,1].map(stars => ({stars, pct: 0}));
